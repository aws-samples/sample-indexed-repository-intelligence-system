...
class DynamoDBMessageLogger:
    ...
    def __init__(
        ...
        Args:
            ...
            ttl_days: Number of days before conversation records expire (default: 90)
        """
        ...
        logger.info(
            f"DynamoDB message logger initialized for table: {table_name} in region: {region_name}"
        )
    ...
    def _write_message_to_dynamodb(
        ...
        try:
            ...
            # Update the record with the new JSON string and TTL
            import time

            ttl_value = int(time.time()) + (self.ttl_days * 86400)
            self.table.update_item(
                Key={"PK": pk, "SK": sk},
                UpdateExpression="SET agent_messages = :messages, expiry_ttl = :ttl",
...
class DynamoDBMessageTracker:
    ...
    def __init__(
        ...
        try:
            self.db_logger = DynamoDBMessageLogger(table_name, region_name=region_name)
            ...
            self.logger.info(
                f"DynamoDB message tracker initialized for job {job_id} in region {region_name}"
            )
        except Exception as e:
